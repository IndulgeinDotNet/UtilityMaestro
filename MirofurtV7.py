...
def create_network_sniffer_tool(tool_frame):
    ...
    # Create a label to explain the color code
    ...
    def packet_capture(packet):
        if not stop_sniffing_flag:
            ...
            # Determine the message type based on packet_info (you can modify this logic)
...
def create_sql_injection_tool(tool_frame):
    ...
    def execute_sql_injection_with_params():
        try:
            # Get the selected payload description and find the corresponding payload
...
info_label = ttk.Label(access_frame, text="Created By IndulgeIn. Version 0.8- Mirofurt", font=("Sans-serif", 7))
info_label.pack()
