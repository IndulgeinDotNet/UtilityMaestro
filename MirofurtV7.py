...
def create_sql_injection_tool(tool_frame):
    ...
    payload_combo = ttk.Combobox(tool_frame, values=[payload["payload"] for payload in sql_injection_payloads], width=50)
    payload_combo.pack()
    ...
    create_button(tool_frame, "Execute SQL Injection", lambda: execute_sql_injection(target_entry.get(), payload_combo.get(), result_text))
...
info_label = ttk.Label(access_frame, text="Created By IndulgeIn. Version 0.778- Mirofurt", font=("Sans-serif", 7))
info_label.pack()
